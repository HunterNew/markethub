...
router.get('/reports/summary', authenticate, requireRole('admin'), async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const [pendingCatRequests] = await conn.query(`SELECT COUNT(*) as cnt FROM category_requests WHERE status = 'pending'`) as any[];
    const totalPendingCats = (pendingCategories as any[])[0].pending_categories + ((pendingCatRequests as any[])[0]?.cnt || 0);
    ...
    return res.json({
      ...
      summary: {
        ...
        pendingCategories: totalPendingCats,
